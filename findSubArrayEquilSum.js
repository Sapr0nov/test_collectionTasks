...
function sostavChisla(massivChisel, chislo) {  
  ...
  const workMassive = massivChisel.filter(num => num < chislo);
  ...
  for(let i = 0; i < uniqueNumbers.length; i++) {
    // добавляем в тестовый массив текущее число
    part.push(uniqueNumbers[i]);
    // сумируем все числа в тестовом массиве
    const summ = part.reduce((a, b) => a + b);
    // если сумма равна числу добавляем в результаты
    if (summ === chislo) {
      results.push([...part]);
    }
    // Если сумма превышает число выкидываем последнее
    if (summ > chislo) {
      part.pop();
      // сдвигаем счетчик на элемент, который был добавлен последним
      i = uniqueNumbers.indexOf(part[part.length - 1]);
      // вытаскиваем ещё один элемент
      part.pop();
    }
  }
